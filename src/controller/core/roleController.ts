...
class RoleController {
  public createRole: RequestHandler = async (req, res) => {
    try {
      ...
      const role = await getRepository(Role)
        .create({ name: name, permissions: permissionsObjs })
        .save();
  ...
  public updateRole: RequestHandler = async (req, res) => {
    try {
      ...
      const permissionsObjs = await getManager()
        ...
        .where('permission.internalId IN (:...permissions)', { permissions })
        .getMany();
      const isRoleExist = await roleRepository.findOne({
        name,
        internalId: Not(roleId),
      });
      if (isRoleExist)
        return res
          .status(EHttpStatusCode.CONFLICT)
      ...
      const role = await roleRepository.update(
        { internalId: roleId },
        { name, permissions: permissionsObjs }
      );
      return res.json(role);
    } catch (error) {
      console.log('error', error);
  ...
  public assignRoleToUser: RequestHandler = async (req, res) => {
    try {
      ...
      const reqUser = req.user as User;
      const neededPermissions = reqUser.role.permissions.filter(
        (p) => p.name === appPermissions.assignUserRoles
      );
      if (!neededPermissions || !neededPermissions.length)
        return res
          .status(EHttpStatusCode.FORBIDDEN)
  ...
  public deleteRole: RequestHandler = async (req, res) => {
    try {
      ...
      if (!role)
        return res
          .status(EHttpStatusCode.BAD_REQUEST)
          .json({ error: 'Invalid Role ID.' });
      if (role.default)
        return res
          .status(EHttpStatusCode.BAD_REQUEST)
          .json({ error: 'You cannot delete default role.' });
      if (role.user && role.user.length)
        return res.status(EHttpStatusCode.BAD_REQUEST).json({
          error: 'You cannot delete this role as it is assigned to some users',
          users: role.user.map((u) => u.username),
        });
